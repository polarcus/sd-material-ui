...
export default class AutoComplete extends Component<Props, State> {
    ...
    filterFunc = (
        ...
        const filteredResults = options.filter((x) =>
            mapFilterToFunc[this.props.filter](inputText, x.label)
        );
    ...
    _updateTextProps = (searchText: string) => {
        ...
        if (typeof setProps === 'function') setProps({searchText});
        if (this.props.searchEndpointAPI !== null)
            fetch(this.props.searchEndpointAPI, {
                body: JSON.stringify({
                    ...this.props.searchJSONStructure,
                ...
                .then((response: {dataSource: Array<any>}) => {
            ...
            <div className={className}>
                <ThemeProvider theme={polarcusTheme}>
                    <MuiAutoComplete
                        ...
                        onChange={(event) => {
                            const val = this.state.dataSourceRender.filter(
                                (opt) => {
                                    return (
                                        event.target.textContent === opt.label
                                    );
                                }
                            )[0].value;
